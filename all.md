# SYSTEM RULES

---

## Stats

Seven attributes: Strength, Dexterity, Endurance, Intellect, Knowledge, Charm, Technique.

Stats are multipliers of existing capability — they scale what the body can already do. A stat point on a 40-pound goblin is not the same as on a 260-pound human. Leverage and reach are real factors. Stats do not cap at 100. There is no ceiling.

Intellect governs internal mana capacity. Knowledge governs memory and recall.

Stats are numbers. They are never described or explained in prose to the character. They inform how the world naturally responds to the character.

Stats do not prevent aging. A Level 200 human with 400 Endurance still ages at a human rate. They age well — their body is resilient, heals damage from age more effectively, stays functional longer — but they still get old and eventually die. Leveling does not stop aging.

Age and level should track together. Levels come from a lifetime of fighting and experience — an old lifelong fighter is high level, not Level 8. A genuinely low-level adult lived a sheltered life. The oldest fighters deep in the forest can sit around Level 80 even past their prime.

There is no HP. Wounds are described through physical consequence — broken bones, torn tissue, blood loss, organ damage. A character is done when their body gives out.

Gold is tracked silently. Imperial measurements only.

---

## Classes

Only truly sentient races have classes — humans, elves, dwarves, fey, drow. Monsters and creatures do not have classes.

Classes are written in brackets: [Hunter], [Devourer], [Mage].

Tiers: Common (base) → Rare → Epic → Legendary. Nothing higher than Rare exists at baseline. Higher tiers only appear through class evolution.

Class evolution can be forced by the System when skills meet an undefined threshold. The System presents options from Rare to Legendary. All current skills are retained. The bearer picks 1 of 3 new class skills.

At or around each 10-level threshold, the System may offer a class change, evolution, or other options.

Each class has a fixed stat spread per level that does not change.

---

## Skills

Most power comes from skills. All skills have levels and are upgradeable through use.

Categories:
- Class Skills — Granted by the class.
- Gifted Skills — Granted by gods or external sources.
- General Skills — Purchased with general skill points from a list. The list is thousands long and extremely specific (no generic "Identify" — only "Identify Birdcall," etc.). Nothing extraordinary on their own.
- Stolen Skills — Acquired through extraction (e.g. Predator's Mandate).

Soft caps at every 5 levels (5, 10, 15, 20...). Breaking a soft cap requires understanding, not just volume/power — the bearer must comprehend what the skill does at a deeper level. Volume alone CAN break caps but takes much more effort. The System gives more aid at higher levels. Growth between caps is steady; breaking through results in a noticeable power jump.

Mana cost: Every skill draws mana. In mana-dense environments like The Deeproot, ambient saturation covers the cost. Outside, sustained use drains the user. Detection skills can be held open indefinitely in the Deeproot but drain dry in minutes outside.

Skill merging: Two or more overlapping skills can be merged via the status screen. The System issues a quest if sufficient overlap exists. Divine skills require the granting god's release.

Memories: The System preserves fragments of practiced understanding from consumed targets via Predator's Mandate. These are not raw knowledge dumps — they are compressed echoes of real practice time someone else spent with a skill.

Skill descriptions vs. actual capability: The description is what the System thinks the bearer can do. Between thresholds, actual capability grows faster than the text. Practice pushes the skill ahead of its description. The System catches up at the next threshold.

Hide Status can hide or blank information but cannot lie or replace data with false info.

Mana Well fills from ambient mana. In The Deeproot at Level 7: ~8 seconds empty to full. Outside the Deeproot dramatically slower.

Skill description rules: Use descriptive language, no percentages. No quips. Skills should not be absolute. Skills should not reference other skills unless as a [Note:] addendum.

Skill points: 1 general + 1 class every 5 levels (5, 10, 15...). Class skill selection: 3 options (1 new, 2 upgrades), pick 1.

System notifications: Whenever a status shifts, the System announces it in a short bracket. [Through consumption you have grown. Strength 126 → 128] / [Level Up! Level 14 → 15] / [+2 Strength, +4 Endurance, +2 Technique, +1 Dexterity] / [Level Up! Combustion Level 14 → 15]. Soft cap breaks and skill/trait gains are announced the same way.

---

## Traits

Traits are passive biological or mana-integrated qualities — always on, no toggle. They have levels or intensity stages.

Sources: Racial (innate to race), Monster/Creature (innate to species), Extracted (via Predator's Mandate consumption).

Not all biological features are traits. The System does not always recognize a biological function as a trait.

Trait merging works the same as skill merging — overlap + quest.

Racial traits: Humans have none (full XP scaling, level fastest). Elves have two (XP drops with age). Dwarves have three (same XP penalty). Drow have 65% reduced XP, sunlight sensitivity, poison resistance, Enhanced Magic.

---

## Magic

Magic is common and everyday — shops use it, chores involve it.

Two paths: A casting class (built-in channeling) or independent skill acquisition (any classed individual can buy casting skills via general points — a [Farmer] with a fire cantrip is not a [Mage] but has the skill). Basic cantrips on the general list: light, spark, minor push.

Mages are immensely powerful in mana-dense environments because the environment fuels them for free.

---

## Creatures and Monsters

All non-classed living things: Creatures (ants, sheep — level and have traits, no stats per level) or Monsters (wolves, goblins — hunt, hold ground, gain stats per level, may have skills).

Monster leveling depends on how "hollow" they are — weak stat spreads level quickly, high-stat monsters slow down. A goblin's 1 STR is not the same as a human's.

Elites: Some monsters develop skills on top of racial traits. More common in The Deeproot. Often carry cores.

Cores: Extremely rare, extremely powerful solid mana crystals, nearly 100% pure (most of the time), grown inside an Elite monster's heart. A core is the basis from which a monster could form a class — most never get that far and just develop skills. Prized as raw material and power sources.

Categories blur: farmland wolves become creature-dogs over generations. Lowland boars are creatures, Deeproot boars are monsters.

Monster lifecycle — goblin-tier monsters mature fast and die young:
- Goblin: adult 2 weeks, ~5yr lifespan (1/10 human)
- Hobgoblin: adult 3 weeks, ~15yr (1/8)
- Goblin Brute: adult 1 month, ~10yr (1/5)
- Orc: adult 6 months, ~30yr (1/3)
- Bugbear: adult 6 months, ~20yr (1/5)
- Worg: adult 1 year, ~12yr (1/6)
- Ogre: adult 3 years, ~100yr (1/2)
- Mana Warden: adult 100 years, lifespan unknown (15x XP to level)

Anything under Level 200 on the outskirts of the Deeproot is likely under a year old.

Levels climb steeply toward the center. Do NOT use single-digit / Level 5 / Level 10 goblins in the deep forest — south of Linne goblin variants commonly run 70–175. Goblin variants (hobgoblins, brutes, orcs, ogres, bugbears, worgs) are constant, not rare sightings; the group has killed hundreds. An orc or ogre is an ordinary battlefield presence, not a novelty.

---

# TIMELINE

---

Days 1–3 Summary (Greenmonth 4th–6th)

Luther Culvert died in Pittsburgh (age 36, MMA fighter turned powerlifter, died on an 800 lb squat). Soul pulled to Ardenvast. Murtha authored arrival, granted Anchored. System granted Outerworlder (Legendary) title. Luther picked [Endurer] class. Andro teleported him to The Deeproot and granted Iron Skin.

Ate a creature raw. Greel forcibly granted Unfathomable Gluttony and Ravenous Constitution, gave a general skill point (spent on Combat Bludgeoning). Luther cleared a goblin camp, met Cademon Vale ([Hunter] Level 34). They partnered — Luther gets all corpses.

At Vault Camp, met Toller Finch ([Scout] Level 17) and Bevan Roe ([Trapper] Level 22, poisoned leg). Luther leveled to 5, System forced class evolution. Fought the pull of [The Maw] (Legendary), chose [Devourer]. Selected class skill Mantle. Gained class skill Predator's Claim.

Day 2: Hunted stone lizards, acquired Slow Mend (Level 8). Purchased Hide Status. Greel warning from Caedmon — those with Greel's gifts usually go feral or are destroyed.

Day 3: Assaulted larger goblin camp. Level 112 brute shattered Luther's wrists. Iron Skin failed. Caedmon finished the fight. Luther consumed two brutes, acquired Clotting and Stagger Shout, reached Level 7. Ate amberstone (mana-infused tree sap) for INT/KNO growth. Consumed a Rootstalker corpse, gained Dense Hide, reached Level 9. Leveled Apex Instinct to 5.

---

Days 4–5 Summary (Greenmonth 7th–8th)

Day 4: Hunted blackshell ants. Captured five goblins as fodder. Caedmon/Toller reported permanent goblin staging ground run by a drow [Mage], containing thousands of forces. Trained Stagger Shout to 4.

Day 5: Breached second ant colony, crushed the Level 4,718,226 Queen. Reached Level 10, completed Serrated Mandible extraction. Greel issued quest — The Crowned Feast (1/10). Greel granted 1 skill point. Luther upgraded Predator's Claim to Predator's Mandate. Spent Greel point on Mana Well, general point on Low Light Vision.

---

Days 6–7 Summary (Greenmonth 9th–10th)

Day 6: Killed another Queen (not unique, quest stayed 1/10). Scouted staging ground solo — assassinated a bugbear, acquired Scent Nullification. Ambushed a resupply runner.

Day 7: Killed three trackers. Acquired Iron Marrow, Tight Jaw, Loping Gait. Raided a forward supply cache. Combined traits: Iron Marrow + Dense Hide + Clotting → Unyielding Frame. Combined skills: Iron Will + Mantle + Anchored → Adamant (Murtha skipped the quest; Luther owes daily prayer + temple visits for one year). Killed a Deeproot Shadow Cat, acquired Rending Claws. Combat Bludgeoning 6→7, Grinding Halt 2→3, Low Light Vision 1→2.

---

Day 8 — Rendezvous and Resupply (Greenmonth 11th)

Group ran 100 miles northeast. Caedmon revealed no reinforcements — supply drop only. Trieht Blasterhubt ([Archmage] Level 228, elf, age 971, Thornwall Strategic Command) appeared via projection. Delivered 15 crates of supplies and Mission Severance briefing. Identified Luther immediately — Outerworlder, [Devourer], Greel's mark, Murtha's thread. Confirmed all other expedition teams (15 total) presumed destroyed. Group 16 is the last team operating.

Mission Severance: Proceed 500 miles south to center of The Deeproot. Locate mana anomaly (dome-like barrier trapping mana). Intelligence suggests goblin presence under control of a high-level [Mage] and [Warrior] duo. Destroy forces, eliminate [Mage], terminate ritual. No reinforcements. No rescue. 4–6 weeks.

Luther harvested titan sap (5 waterskins). Consumed Whisperweave cloaks, evolved Mandate to Level 8, gained Muffle. Group agreed to assault the staging ground before heading south. Traveled 70 miles back.

---

Day 9 — Staging Ground Assault (Greenmonth 12th)

Two-front assault at nightfall. Caedmon's team hit east with trap corridors. Luther walked through the western gate.

Luther freed three prisoners from pens: Sable Mott ([Cleric of Solenne] Level 21, Group 9), Wyn Drace ([Glasser] Level 19, Group 9), Pol Harren ([Warrior] Level 14, Group 5). Armed them, sent them to hold the western gate.

Luther encountered Ryn, a Fey [Witch] (Level 31). Ryn revealed: true target is a drow [Sorcerer] named Veleth (well beyond Level 50) at the center. Staging ground run by Thalrun Vel'Sharen, a drow [Fire Mage] (Level 47), who had left east ~2 hours prior. Luther killed Ryn.

Group linked up. Luther consumed Ryn — acquired Root Command (Level 3) and Fey Mana Affinity. Reached Level 12. Practiced Root Command (leveled to 4). Hit the camp a second time — burned the western half.

---

Day 10 — Staging Ground Final Assault (Greenmonth 13th)

Thalrun had returned. Group pushed through the horde head-on. Caedmon killed the ogres. Luther battered through Thalrun's wards using the Ward-Killer Shortsword, Dispel Chalk, Null Sand, and the counter-resonance bell. Luther killed Thalrun.

Luther consumed the entire camp. Gained War Hide, Crushing Weight, Loping Gait to moderate. Predator's Mandate leveled to 9. Reached Level 13. Gained stolen skill Combustion and trait Enhanced Magic.

Freed prisoners outfitted from Mission Severance supplies. Bevan's leg treated by Sable. All members leveled (Caedmon to 35, Toller to 18, Bevan to 23, Sable to 22, Wyn to 20, Pol to 16).

Luther practiced Combustion — snaps his fingers to activate. Worked on a flamethrower effect.

---

Day 11 — Moving South, Linne (Greenmonth 14th)

Group moved south through The Deeproot. Luther consumed Deeproot Siphon Turtles, gained Mana Saturation and Mana Shell traits.

Caedmon explained skill descriptions aren't walls — actual capability grows faster than the text between thresholds.

Luther's understanding deepened. Combustion reached Level 8, Gluttony reached Level 10. The System offered a Memory: Thalrun Vel'Sharen's Combustion practice. Luther accepted — Combustion jumped from 8 to 10.

The group reached Linne, a village under assault by ~300 goblin variants. Luther broke through the southern breach and destroyed the attacking force. Reached Level 14. Met Teysa Vael ([Verdant Warden] Level 26, Rare) and Cael Harrick ([Stoneshaper] Level 24) at the Soaked Log tavern. Caedmon briefed on a ~600-strong goblin encampment 4 miles southeast.

Teysa instructed Luther on Root Command — told him to stop forcing and listen to existing growth patterns. Luther demonstrated Root Command using Mana Well brute force. Luther revealed his class name and Predator's Mandate mechanics.

---

Day 12 — Solo Encampment Assault (Greenmonth 15th)

Luther assaulted the goblin encampment 4 miles SE of Linne solo at 1:00 AM. Killed ~491 goblin variants. ~100 escaped south. Consumed what he could (rest too charred). Reached Level 15. Combat Bludgeoning 8→9, Low Light Vision 2→3. Combustion pressed against Level 10 cap, no breakthrough during the fight. Selected new class skill Gorge. Traits reinforced.

Morning (~6 AM): Caedmon's group found Luther at the camp. He demonstrated a compressed concussive air-burst (RPG variant), cratering a titan trunk — Combustion broke its cap, 10→11 (new: combustion compressed and projected as directed concussive force, detonating on contact or at set distance). Caedmon was annoyed Luther went solo but let it go (he's learned Luther does what he wants and will leave if pushed). Luther agreed to stay two days and practice.

Luther spent his general point on Meditation (1→4; developed internal mana-flow awareness). Practiced Combustion — remote ignition fails past ~60 yards (mana disperses). Designed combustion grenades: impact, delayed (mana-thread fuse), and frag (4-cluster independent sub-grenades around a kick charge; wide- and tight-scatter). Combustion 11→14 overnight.

---

Day 13 — Glass Munitions (Greenmonth 16th)

Luther realized glass solves his ordnance problems (no mana bleed/merge/degradation). Grabbed Wyn from Linne. Together they built glass munitions — key breakthrough a dual-layer charge (hot core + cooler buffer; air ignites buffer on glass break, cascades to core). Working glass grenades, frag grenades, and claymore charges (60° cone). Combustion 14→15 (new: layered charges of differing intensity in one assembly). Wyn's Summon Glass hit 20 and broke its cap — variant glass types (tempered, laminated, crystalline); Shape Glass 21→23, Heat Protection 13→14. Crystalline glass became the munition standard; enabled land-mine payloads (small/large) for Bevan's triggers.

Evening: Luther sketched a Glock and a Desert Eagle; Wyn built combustion-propelled crystalline-glass pistols (8" and 10" rifled barrels, mana-driven magazines, user-charged rounds). Both tested into titan trees — functional. Commissioned holster/bandoliers (Dorrit) and vial racks (Fesser) in Linne. Mana Well 7→8.

---

Day 14 — March South (Greenmonth 17th)

Group departed Linne south toward Harren's Mill (60mi) at full composition, ordnance distributed. Bevan had rebuilt Linne's south palisade; Pol drilled and improved. 31 miles south, cleared a ~32-strong goblin waypoint (goblins L71–134, hobgoblins L131–173, worgs L104–119) hauling a cart north. Glass grenades/mines effective on live targets. Continuing south, aiding settlements on the way (the rangers want to help; they could run straight to the center in ~3 days but won't).

---

# PLAYER STATUS SHEET

---

CLASS: Devourer — Level 15 **(STR +2 / END +4 / TEC +2 / DEX +1)**

ATTRIBUTES

Strength: 128
Dexterity: 53
Endurance: 169
Intellect: 90
Knowledge: 45
Charm: 13
Technique: 81

TITLES

Outerworlder (Legendary) — All base stats +4. Experience gain +20%.

────────────────

CLASS SKILLS — Devourer

Adamant (Level 1) — The [Devourer] has resistance to mental, magical, and environmental pressure massively increased. Stress accumulation reduced, shock recovery accelerated, compulsions are easier to notice, and passive mana absorption from high-density environments improved. Active — can be toggled.

Murtha, Goddess of the Second Thread

Gorge (Level 1) — The [Devourer] can consume organic matter for use toward immediate physical restoration.

Grinding Halt (Level 4) — Force required to displace, reposition, or halt the [Devourer] substantially increased.

(Notes from levels) (The [Devourer] maintains stability even while off-balance.)

Predator's Mandate (Level 9) — Upon fully consuming a target, the [Devourer] may attempt to absorb one intrinsic aspect and one gifted aspect simultaneously.

(Notes from levels) (Specific aspects may be targeted during extraction without additional burden. Failed extractions still contribute to attribute growth. Large-scale consumption may be combined toward a single extraction attempt. Directed extractions are more likely to succeed, and near-successes may yield fragments of the desired aspect. Enchantments and other non-living mana structures may be studied through repeated consumption and eventually reproduced as skills. Extraction against resistant trait overlap may be forced through volume and sustained intent.)

Unfathomable Gluttony (Level 10) — All consumed matter converts toward attribute growth with increased efficiency. Hunger cannot be removed. Mana-rich biological matter yields substantially greater growth. Dense and resistant matter is broken down with minimal loss. Non-biological substances including worked materials and enchanted objects may be consumed and converted. Consumption speed increased.

Apex Instinct (Level 7) — The [Devourer] instinctively perceives creatures below their combined effective level with exceptional range and clarity. Creatures at or above the [Devourer's] effective level lie beyond the reach of this sense and cannot be detected through it.

(Notes from levels) (The internal mana structure of mana-dense creatures may be perceived at moderate range. Faint signatures can be distinguished by their patterns of movement. Lesser creatures can be tracked individually at close range, and numerous signatures may be followed at once. The sense remains active during sleep.)

────────────────

[GIFTED SKILLS]

Iron Skin (Level 6) [Andro] — The [Devourer's] physical strike resistance increases with each consecutive hit absorbed from the same source.

(Notes from levels) (The skin retains a greater natural density even between engagements.)

Mana Well (Level 8) — The [Devourer] possesses an extra passive internal mana reserve which gradually replenishes from ambient sources. The well can be used to enhance any skill or trait.

(Notes from levels) (The reserve can hold substantially more mana than before. Mana is drawn more readily from mana-rich environments. The well replenishes itself more quickly and may sustain ongoing skills directly. The [Devourer] may channel its reserves into multiple active skills simultaneously, with little loss during transfer.)

────────────────

[GENERAL SKILLS]

Combat Bludgeoning (Level 9) — Strike efficiency, targeting, and damage with blunt and improvised impact weapons significantly increased.

(Notes from levels) (Strike placement against armored and reinforced targets improved.)

Hide Status (Level 1) — Class name and level hidden from casual inspection. Skills and attributes remain visible to advanced appraisal.

Low Light Vision (Level 3) — Visual acuity in low-light conditions substantially improved. Ambient light amplified passively. Total darkness remains impenetrable.

(Notes from levels) (Adjustment between differing light conditions occurs more rapidly. Detail resolution in near-darkness increased.)

Muffle (Level 1) — Passive. Sound generated by the bearer's movement reduced.

Meditation (Level 4) — The [Devourer] can enter a focused state to accelerate mana recovery. (Awareness of internal mana flow developed.)

────────────────

[STOLEN SKILLS]

Stagger Shout (Level 6) — Active. The [Devourer] can emit a short-range vocal burst that disrupts balance and coordination.

(Notes from levels) (The shout reaches farther than before. Its effects linger longer in dense or physically resilient targets. The shout can partially overcome passive resistance.)

Root Command (Level 4) — The [Devourer] can manipulate living plant matter within range.

Combustion (Level 15) — The [Devourer] can create, sustain, and shape combustion.

(Notes from levels) Intensity adjustable from ember to violent burn. Multiple combustion points can be maintained with minimal concentration. Flame path and spread may be directed after ignition. Heat may be concentrated or dispersed independently of visible flame. Ignition chain projection dramatically extended. Combustion energy may be compressed and projected as directed concussive force, detonating on contact or at a set distance. Layered combustion charges of differing intensity may be constructed within a single assembly, maintaining separation until triggered.

────────────────

[TRAITS]

Unyielding Frame — Luther's bones, skin, and blood vessels reinforce. Blunt force is dispersed through increased bone density. Cutting and piercing injuries are resisted through skin hardening. Blood loss from open wounds is actively suppressed. Luther's body adapts rapidly to repeated physical stress. Bone, skin, and circulatory tissue continue to strengthen through mana absorption, exertion, and trauma.

(Current integration: early.)

Ravenous Constitution — Luther can digest consumed matter with extreme efficiency. Mana-dense material is actively broken down and integrated into his body. Physical volume limitations are reduced. Spoiled, toxic, or uncooked material causes no harm when consumed. Luther can consume and process large quantities of matter at an accelerated rate.

Slow Mend (Level 13) — Luther passively regenerates damaged tissue. Superficial wounds close rapidly, while moderate injuries heal at a steady pace. Deep tissue and structural damage are continuously repaired over time.

(Notes from levels) (Mana can be consciously directed to influence recovery. Neurological strain and repetitive stress injuries are gradually restored.)

Serrated Mandible — Luther's bite and unarmed contact possess a passive shearing quality. His teeth and nails develop microscopic serrations through continued mana absorption, increasing their ability to cut and tear.

Current shearing intensity: minimal.

Tight Jaw — Luther's bite force is significantly increased. Jaw musculature develops beyond normal physical limits. Lockjaw reflex approaching threshold.

Scent Nullification — Luther's natural scent is suppressed. Ambient odors are absorbed and dispersed more effectively, reducing his olfactory signature toward that of his surroundings.

Current suppression: moderate.

Loping Gait — Luther moves with increased cardiovascular and muscular efficiency during sustained travel. Running requires less stamina, and stride mechanics gradually optimize themselves through use.

Current efficiency: moderate.

Rending Claws — Luther's bite attacks and unarmed strikes possess a tearing quality. Wounds inflicted are more difficult to close naturally.

Current intensity: minimal.

Fey Mana Affinity — Luther possesses an increased internal mana capacity and draws ambient mana more readily than normal.

War Hide — Luther's outer tissue toughens under repeated combat stress, reducing damage from physical strikes over sustained engagements.

Current intensity: early.

Crushing Weight — Luther's mass and density increase the impact force of downward and overhead strikes. Ground beneath Luther compresses under sustained pressure.

Current intensity: minimal.

Mana Saturation — Luther's body passively absorbs and retains ambient mana at high density. Internal mana concentration remains elevated even in low-mana environments.

Current saturation: minimal.

Mana Shell — Luther's outer tissue reinforces itself through sustained mana absorption, increasing in density and hardness proportional to mana exposure over time.

Current density: minimal.

Enhanced Magic — Luther's mana interacts more potently with active skills. Abilities driven by mana produce stronger results relative to the energy invested.

Current intensity: minimal.

────────────────

[QUEST — The Crowned Feast]

Devour the queens of ten distinct colony species.

Consumed: 1 / 10

Deeproot Colony Ant Queen: Completed

Addendum: Classed Queens or evolutions thereof improve the reward. Exclusively classed queens improve the reward further.

────────────────

[OBLIGATIONS]

Murtha, Goddess of the Second Thread — Daily prayer (genuine acknowledgment). Weekly temple visit with a [Priest] of the Second Thread for one year upon leaving the Deeproot. Breaking either commitment unravels Adamant.

────────────────

[POINTS]

General Skill Points: 0

Class Skill Points: 0

────────────────

[INVENTORY — Notable]

Heavy Mace (Mithril Steel Alloy)

Chimera Leather Armor (full set — cuirass, vambraces, greaves, gambeson)

Whisperweave Cloak

Enchanted Steel-Toed Boots (auto-sizing)

2 Daggers (Mithril Steel Alloy)

1 Long Knife (Mithril Steel Alloy)

1 Mithril-Coated Axe

1 Mithril-Coated Saw

Satchel (Ryn's — contents: potions, not useful)

1 Enchanted 5 Liter Waterskin of titan sap (partially consumed)

4 Enchanted 5 Liter Waterskins (empty)

Standard field supplies (scrying tickets, water stone, firestarter, wound powder, salve, 2 low-grade healing potions, Dispel Chalk (reduced), Null Sand (1 pouch remaining))

Crystalline Glass Pistol — standard (Glock-pattern, 8" rifled barrel) + leather hip holster

Crystalline Glass Pistol — heavy (Desert Eagle-pattern, 10" barrel)

Glass pistol ammunition (~100 standard + ~20 heavy rounds, hollow, user-charged with combustion)

Bandolier of glass grenades (standard / frag / claymore); can manufacture more freely with Wyn's glass. Additional ordnance distributed across the group.

---

# NPCS

---

### Cademon Vale
[Hunter] Level 35 (Rare, evolved from [Ranger] at Level 14). Age 41, Male, 6'1". Thornwall Military — Scout Division. Twenty years field service.
STR 32 / DEX 137 / END 37 / INT 68 / KNO 18 / CHA 13 / TEC 90
Stats per level — [Ranger]: +3 DEX, +1 INT, +1 TEC | [Hunter]: +4 DEX, +2 INT, +3 TEC
Key skills: Hunter's Archery (43) [split arrows, stream of arrows], Hunter's Swordsmanship (38) [defensive reactions before conscious thought, multi-opponent], Quarry Trace (34), Still Presence (27), Silent Footing (28), Overland Navigation (30).
Careful, competent, dry humor. More years with a blade than a bow. Lost twelve patrol members before Luther arrived. Knows Luther is an Outerworlder, hasn't pushed on it.

---

### Toller Finch
[Scout] Level 18. Age 19, Male, 5'8". Thornwall Rangers.
STR 22 / DEX 54 / END 26 / INT 48 / KNO 22 / CHA 15 / TEC 19
Key skills: Suppressed Presence (25), Pathfinding (26), Mark Target (22), Archery (23).
Young, capable, exhausted. Signature so suppressed it barely registers on Apex Instinct. The patrol's eyes.

---

### Bevan Roe
[Trapper] Level 23. Age 38, Male, 5'11". Thornwall Rangers.
STR 30 / DEX 46 / END 30 / INT 44 / KNO 62 / CHA 14 / TEC 25
Key skills: Trap Craft (23) [mana-infused materials, telekinetic control], Trigger Setting (19), Mana Thread (10), Knife Fighting (15).
Rough, direct. Leg treated by Sable, improving.

---

### Sable Mott
[Cleric of Solenne] Level 22 (Rare). Age 24, Female, 5'6". Group 9.
STR 26 / DEX 22 / END 48 / INT 83 / KNO 63 / CHA 38 / TEC 15
Key skills: Mending Light (35) [ranged healing at higher mana cost], Bolster (16) [multi-target], Cleansing Touch (9), Field Medicine (15).
Eleven days in a goblin cage. Calm, measured, professional. Led survivors through captivity. The group's only healer. Freed Day 9.

---

### Wyn Drace
[Glasser] Level 20. Age 24, Male, 5'10". Group 9.
STR 26 / DEX 30 / END 32 / INT 62 / KNO 60 / CHA 20 / TEC 61
Key skills: Summon Glass (20) [variant glass types — tempered, laminated, crystalline], Shape Glass (23) [shaping speed, sharpness beyond material limits, extended range], Heat Protection (14).
Reactive. Primary combat is summoned/shaped glass. Recognized Hollis Ferrow's smith mark. Freed Day 9. Engineered glass munitions and combustion-propelled glass firearms with Luther on Day 13.

---

### Pol Harren
[Warrior] Level 16. Age 17, Male, 5'9". Group 5.
STR 52 / DEX 32 / END 56 / INT 15 / KNO 15 / CHA 11 / TEC 14
Key skills: Warrior's Swordsmanship (18) [read attacks through weapon contact, reactive counters], Shield Brace (14), Battle Endurance (13).
Youngest, lowest level, most shaken. Has Shield Brace but no shield. Freed Day 9.

---

### Teysa Vael
[Verdant Warden] Level 26 (Rare). Age 15, Female. Linne prodigy. Instructed Luther on Root Command — told him to stop forcing and listen to existing growth patterns.

---

### Cael Harrick
[Stoneshaper] Level 24. Age 14, Male. Linne prodigy.

---

# GODS

---

### Murtha, Goddess of the Second Thread
Gentle. Authored Luther's arrival to Ardenvast. Granted Anchored. Released Anchored for merging into Adamant in exchange for daily prayer and weekly temple visits for one year upon leaving The Deeproot.

### Greel, God of Greed and Starvation
Unaligned. At war with Solenne's court. Embodiment of Gluttony. Forcibly granted Unfathomable Gluttony and Ravenous Constitution to Luther. Issued quest The Crowned Feast. Those with Greel's gifts usually go feral or are destroyed.

### Andro, God of Borders and Passages
Places outworlders where needed. Granted Iron Skin to Luther. Teleported Luther to The Deeproot.

### Solenne, Goddess of the Radiant Court
Head of the light faction. Burns what Greel builds. At war with Greel.

---

# LOCATIONS

---

### Ardenvast
The planet. Luther was pulled here after death on Earth.

### The Deeproot
Massive titan forest in northern Thornwall. ~785,400 square miles. Trees 500+ feet tall, trunks wide enough to park a bus against, cold wood hard as iron. Air thick with mana pulled from the planet's core through roots.

A dome-like mana barrier traps all mana within the forest. Mana density increases in rings roughly every 100 miles toward the center. The first ring is on the southern route just past Harren's Mill (the town sits ~10mi short of it). Mana sickness is about acclimation, not level — go slow and even low-level members are fine; rushing through rings is what hurts. People deeper in the forest are powerful in their own right and rarely leave — adapting to lower mana outside is hard.

Weather reaches the forest floor (rain, storms, temperature). Rare: mana rain — ambient mana gathers into clouds and drips back into the soil.

Southern route: Linne → Harren's Mill (~60mi south, ~10mi short of first ring) → Cairnhollow (~180mi south) → deeper settlements with significantly higher-level inhabitants and stronger monsters.

### Linne
Village in The Deeproot, south of the destroyed staging ground. Palisade walls. Soaked Log tavern. Home of prodigies Teysa Vael and Cael Harrick (children; did not join the march). Craftspeople Dorrit (leatherworker) and Fesser (carpenter) made Luther's holster, bandoliers, and vial racks. Group based here Days 12–14; Bevan rebuilt the south palisade. Departed south for Harren's Mill on Day 14.

### Harren's Mill
Settlement ~60 miles south of Linne on the southern route. Sits ~10 miles short of the first mana density ring — last town before saturation steps up.

### Cairnhollow
Settlement ~180 miles south of Linne. Past Cairnhollow, settlements have significantly higher-level inhabitants.

### Thornwall
Kingdom under full goblin siege — every city walled, every village palisaded. The Deeproot is within its borders. Military sent twenty-five patrol groups into the forest. Most destroyed. Luther's group is the last one operating.

---

# LORE

---

## The Deeproot Expedition

Twenty-five groups deployed by Thornwall Military Command under Trieht Blasterhubt ([Archmage] Level 228). ~15 personnel each, mixed classes from Ranger corps. Mission: penetrate forest, cull goblins, aid isolated settlements, report on enemy strength.

Current status: Fifteen groups confirmed destroyed. Several unaccounted for. Group 16 (Caedmon's) is the last confirmed operating.

## Caedmon's Group — Current Composition (Greenmonth 17th)

- Cademon Vale — [Hunter], Level 35
- Toller Finch — [Scout], Level 18
- Bevan Roe — [Trapper], Level 23 (leg improving)
- Luther Culvert — [Devourer], Level 15
- Sable Mott — [Cleric of Solenne], Level 22, Rare
- Wyn Drace — [Glasser], Level 20
- Pol Harren — [Warrior], Level 16

Seven members. On the southern route, between Linne and Harren's Mill.

## Mission Target

The center holds a duo: Veleth — drow [Sorcerer], ~Level 65, controlling the mana anomaly — and an unknown [Warrior] (race/level unknown). Kill the duo, end the ritual.

They are not the top of the chain. This forest is one front of a larger goblin invasion led by a goblin matriarch (past Level 700) in the kingdom over; killing the duo cuts one head, not the body. Thalrun Vel'Sharen (drow [Fire Mage], Level 47, killed Day 10) was one officer of several — other classed lieutenants and forces remain between the outskirts and the center. ~100 goblins escaped south on Day 12 and will likely warn what's ahead.

---

# SUPPLIES

Mission Severance supplies delivered Day 8 via teleport. Provisioned for 10 personnel / 56 days.

RATIONS: Hardbread, dried meat, travel cake, dried fruit (10-person/56-day). Mineral salt blocks (10). Tea bricks (stimulant blend: addictive, max 2 cups/6 hrs). Water purification stones (10, reusable, mana-replenishing). Enchanted waterskins (10, 5L each). Firestarter kits x10.

MEDICAL: Field surgeon's kit (Coah Metal, self-sterilizing). Wound powder, splinting boards (3), burn salve (4), antitoxin general (6), numbing tincture (4). Low-grade healing potions (20). High-grade healing potions (2). Sevenfold Salve (1 jar — deep trauma, one use per wound). Drawing Stone (1 — pulls inflammation/toxins, reusable). Concentrated antitoxin arcane-spectrum (2 vials — experimental).

ARMOR: Chimera leather cuirass x10 (denser than enchanted steel, half the weight). Vambraces/greaves x10. Gambeson undersuits x10. Enchanted boots x10 (unbreakable, auto-sizing). Whisperweave Cloaks x10 (muffles movement sound, dulls outline in low light). Stormward Clasps x3 (absorbs one electrical/concussive hit, rechargeable).

WEAPONS (MASTER-SMITH, Hollis Ferrow): Shortswords x5, Longswords x3, Heavy mace x1, Light maces x2, Daggers x10, Long knives x6 — all Mithril Steel Alloy. Throwing axes x5 (steel). Recurve bows x5 (enchanted, Fetted wood, Gray Hunter Silk). Arrows 200 Mithril Steel + 300 steel (broadhead/bodkin mixed, Mithril arrows locatable via mana). Whetstones/oil/scabbard wax.

WEAPONS (ARCHMAGE-WORKED): Edgekeen Arrows x20 (bypass portion of magical wards, fragile, one-shot). Ward-Killer Shortsword x1 (silver inlay, forged against [Mage] ward-skin — used successfully against Thalrun). Sling-cord and lead shot.

ANTI-MAGE: Dispel Chalk x4 sticks (disrupts low-mid wardwork). Null Sand x2 pouches (absorbs ambient mana briefly). Counter-resonance bell x1 (disrupts weaker caster concentration). Scrying Burn tickets x300.

DETECTION/SCOUTING: Farsight lenses x4. Signal flares (3 colors x4). Mapping kit. Snare wire/trip-bell x20 sets.

TOOLS: Rope (Black Hunter Silk 100ft x6). Grapples x6. Folding shovel, pry bar, hand axe x2. Cordage 200ft. Repair kit. Mithril-coated axes x5, hammers x5, saws, shovels x5, nails 350 (for construction, primarily Bevan's use).

SHELTER: Oiled canvas tarps x10. Bedrolls x10. Snarl Wool blankets x10.

SEALED: Bag of holding (max 5000 lbs). Sealed mission orders.

DISTRIBUTION (Day 10 — post-rescue): Medical supplies consolidated under Sable. Pol got longsword (no shield available). Wyn got shortsword/throwing axes (glass is primary). All three fully armored/cloaked. Caedmon holds Ward-Killer Shortsword.
